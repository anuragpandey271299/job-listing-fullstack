...
export default function AddJob({ forUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!forUpdate) {
      ...
      try {
        ...
        if (jobType) updatedData.jobType = jobType;
        if (officeType) updatedData.officeType = officeType;
        if (location) updatedData.location = location;
        if (jobDescription) updatedData.jobDescription = jobDescription;
        if (aboutCompany) updatedData.aboutCompany = aboutCompany;
        if (skills) updatedData.skills = skills;
        if (info) updatedData.info = info;
        const response = await axios.put(`http://localhost:5000/updatejob/${USERID}`, updatedData , config)
        console.log(response.data)
        if (response.status === 200) {
          toast('Job Updated')
          navigate('/jobfinder')
  ...
  return (
    <div className='AddJobContainer'>
      <form onSubmit={handleSubmit}>
        <h1>{forUpdate ? 'Update Job Description' : 'Add Job Description'}</h1>
